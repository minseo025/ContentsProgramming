...
public class RealWeatherGraph : MonoBehaviour
{
    ...
    // 최고/최저 온도 표시 추가 (추가 기능 구현)
    void DisplayStatistics()
    {
        float maxTemp = monthlyTemps[0];
        float minTemp = monthlyTemps[0];
        int maxMonth = 0;
        int minMonth = 0;

        for (int i = 1; i < 12; i++)
        {
            if (monthlyTemps[i] > maxTemp)
            {
                maxTemp = monthlyTemps[i];
                maxMonth = i;
            }
            if (monthlyTemps[i] < minTemp)
            {
                minTemp = monthlyTemps[i];
                minMonth = i;
            }
        }

        Debug.Log("===== 2024년 서울 날씨 통계 =====");
        Debug.Log($"가장 더운 달: {maxMonth + 1}월 ({maxTemp}°C)");
        Debug.Log($"가장 추운 달: {minMonth + 1}월 ({minTemp}°C)");
        Debug.Log($"연간 온도 차이: {maxTemp - minTemp}°C");
    }
